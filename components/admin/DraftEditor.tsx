...
export function DraftEditor({
  ...
  async function call(action: 'save' | 'publish' | 'reject') {
    ...
    try {
      ...
      // Throttle preview removed — the AudiencePicker is now throttle-aware
      // and only surfaces eligible subscribers in the first place. The server-
      // side filter at /api/admin/drafts/[id] is still in place as a safety
      // net for edge cases (subscriber crossed the 7-day boundary between
      // page load and send), but doesn't need a confirm dialog — the picker's
      // recipientCount IS the count being sent.
